...
<body class="bg-bg-dark-primary text-secondary-text flex p-0 pt-12 m-0 min-h-screen flex-col">
  <nav class="flex gap-1 p-1 pl-3 bg-bg-dark-nav fixed w-full top-0 h-12 items-center shadow-[0_0_20px_var(--color-body-cyan-40)]">
    <div class="flex gap-3 h-8 ml-auto">
        ...
        <div onclick="window.location.href='{{ route('UVuser_homepage.show') }}'" class="h-8 w-8 rounded-full hover:border group relative">
        ...
        <div onclick="window.location.href='{{ route('profile.show') }}'" class="h-8 w-8 rounded-full hover:border flex justify-center items-center group relative">
          ...
          <span class="rounded opacity-0 9text-xs text-nowrap absolute -left-4  top-[150%] bg-dark-nav text-primary-bright p-1 after:content-[''] after:absolute after:right-2 after:bottom-full after:border-[5px] after:border-transparent after:border-b-accent-rose group-hover:opacity-100 pointer-events-none">profile</span>
        </div>
    </div>
  </nav>
  <main>
